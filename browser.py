...
class headless_browsers:
    def __init__(self):
        self.opts = Options()
        self.opts.add_argument("--headless") # Para hacer Firefox invisible (modo Headless)
        self.browsers = {}  # Donde guardamos todos los navegadores creados
        self.useragent = ""
        self.browser_path = "/usr/bin/firefox"  # Aseguramos que use esta ruta
        ...
        if err or not out.decode():
            print("No se encontró Firefox en la ruta predeterminada.")
            # Si Firefox no se encuentra, puedes manejarlo aquí (por ejemplo, mostrar un mensaje de error).
        else:
    ...
    def new_session(self, module_name, url, useragent="(random)", force=False):
        ...
        try:
            ...
            # Usamos la ruta forzada para Firefox
            caps['binary_location'] = self.browser_path
            if Settings.debug:
                new_headless[module_name]["Controller"] = Firefox(profile, executable_path="/usr/local/share/geckodriver", capabilities=caps)  # Aquí iniciamos Firefox con la ruta definida
            else:
                new_headless[module_name]["Controller"] = Firefox(profile, executable_path="/usr/local/share/geckodriver", capabilities=caps, options=self.opts)
